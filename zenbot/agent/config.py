...
def load_settings() -> Settings:
    ...
    llm_model = _env_or(llm_config, "ZENBOT_MODEL", "model")
    llm_stream_raw = _env_or(llm_config, "ZENBOT_STREAM", "stream")
    llm_think_raw = _env_or(llm_config, "ZENBOT_THINK", "think", False)
    ...
    llm_settings = LLMSettings(
        model=llm_model,
        stream=_bool_value(llm_stream_raw, "ZENBOT_STREAM"),
        think=_bool_value(llm_think_raw, "ZENBOT_THINK"),
    )

    agent_config = merged["agent"]
    agent_max_internal_steps_raw = _env_or(agent_config, "ZENBOT_MAX_INTERNAL_STEPS", "max_internal_steps")
    agent_max_history_raw = _env_or(agent_config, "ZENBOT_MAX_HISTORY_MESSAGES", "max_history_messages")
    agent_debug_raw = _env_or(agent_config, "ZENBOT_DEBUG", "debug", False)
    ...
    if agent_max_internal_steps_raw is None:
        raise ValueError("max_internal_steps not configured. Set ZENBOT_MAX_INTERNAL_STEPS or config agent.max_internal_steps")
    if agent_max_history_raw is None:
        raise ValueError("max_history_messages not configured. Set ZENBOT_MAX_HISTORY_MESSAGES or config agent.max_history_messages")
    ...
    if agent_max_internal_steps <= 0:
        raise ValueError(f"max_internal_steps must be greater than zero, got {agent_max_internal_steps}. Set ZENBOT_MAX_INTERNAL_STEPS or config agent.max_internal_steps.")
    if agent_max_history < 2:
        raise ValueError(f"max_history_messages must be at least 2, got {agent_max_history}. Set ZENBOT_MAX_HISTORY_MESSAGES or config agent.max_history_messages.")
    if agent_reminder_poll <= 0:
        raise ValueError(f"reminder_poll_seconds must be greater than zero, got {agent_reminder_poll}. Set ZENBOT_REMINDER_POLL_SECONDS or config agent.reminder_poll_seconds.")

    agent_debug = _bool_value(agent_debug_raw, "ZENBOT_DEBUG")
